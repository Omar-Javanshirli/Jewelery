...
{
    [AuthLoginCustom]
    public class DashboardController(IWebHostEnvironment env, JeweleryEntities.JeweleryDbContext context) : Controller
    {
        private readonly IWebHostEnvironment env = env;
        private readonly JeweleryEntities.JeweleryDbContext  _context = context;
        ...
        public ActionResult Index()
        {
            ...
            data.Ads = _context.Ads.Where(w => w.Status == 2).OrderByDescending(o => o.PublishDate).ToList();
            data.TotalAds = _context.Ads.Count();
            data.ActiveAds = _context.Ads.Where(w => w.Status == 1).Count();
            data.WaitingAds = _context.Ads.Where(w => w.Status == 2).Count();
            data.NewMessages = _context.Messages.Where(w => w.IsReaden == false).Count();
            data.TotalUsers = _context.Users.Count();
            return View(data);
        }
        ...
        public ActionResult Edit(int id)
        {
            ...
            data.Ad = _context.Ads.FirstOrDefault(f => f.Id == id && f.Status != 3);
            if (data.Ad != null)
            {
                data.menuItems = _context.MenuItemLanguages.Where(w => w.MenuItem.Status == true && w.LangId == 1).ToList();
                data.menuCategories =
                    _context.CategoryLanguages.Where(w => w.Category.Status == true && w.LangId == 1).ToList();
                data.subCategories = _context.SubCategoryLanguages.Where(w => w.SubCategory.Status == true && w.LangId == 1)
                    .ToList();
                data.subSubCategories = _context.SubSubCategoryLanguages
        ...
        {
            Ad ad = _context.Ads.FirstOrDefault(f => f.Id == id && f.Status == 2);
            if (ad != null)
            {
                ad.Status = 1;
                db.SaveChanges();
